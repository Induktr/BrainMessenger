...
3. Интеграция в контроллер:
   ...
   async uploadImage(@UploadedFile() file: Express.Multer.File, @Inject(StorageService) storageService: StorageService) {
     const processedImage = await this.imageProcessingService.processImage(file.buffer, 300, 300)
     const fileName = `image-${Date.now()}.webp`
     const fileUrl = await storageService.uploadFile(processedImage, fileName)
     return { fileUrl }
   }
   ```
...
### 4. Оптимизация базы данных (Neon/PostgreSQL)
...
**Инструменты:** PostgreSQL (Neon), репликация, шардинг.
...
**Шаги по настройке Master-Slave репликации в Neon/PostgreSQL:**

1. **Настройте мастер:**
   - В Neon репликация настраивается автоматически на платных тарифах. Обратитесь к документации Neon для включения репликации.
...
- В Neon репликация может быть настроена автоматически — проверьте их документацию.
...
1. **Установите Citus:**
   - Neon не поддерживает Citus напрямую, поэтому вам нужно развернуть PostgreSQL с Citus отдельно.
...
2. **Настройте шарды:**
   - Создайте распределённую таблицу:
     ...
     ```
3. **Подключите Prisma:**
   - Используйте одну точку подключения, так as Citus автоматически распределяет запросы.
