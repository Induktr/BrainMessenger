...
export const isCodeSnippet = (text: string): boolean => {
  ...
  if (symbolCount > 3 || (text.length > 0 && symbolCount / text.length > 0.1)) {
    return true;
  }
  ...
  const keywords = [
    ...
    'public', 'private', 'static', 'void', 'int', 'string', 'func', 'def', 'self', 'this', 'typeof', 'type', 'interface', 'string', 'boolean', 'any', 'void', 'null', 'undefined', 'number', 'Date', 'foreach', 'do while', 'try', 'catch', 'finally', 'get'
  ];
